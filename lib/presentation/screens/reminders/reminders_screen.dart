...
class _RemindersScreenState extends State<RemindersScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final upcoming =
          reminders.where((r) => !r.isCompleted && r.date.isAfter(now.subtract(const Duration(days: 1)))).toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(controller: _tabController, tabs: const [Tab(text: 'Upcoming'), Tab(text: 'Completed')]),
      ),
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              isUpcoming ? 'Add reminders to track your pet\'s care schedule' : 'Completed reminders will appear here',
      ...
      child: Column(
        children: [
          ListTile(
            ...
            subtitle: Text(pet != null ? pet.name : 'Unknown Pet'),
            trailing: Checkbox(
              value: reminder.isCompleted,
              onChanged: (value) {
                if (value != null) {
                  _reminderService.updateReminderStatus(reminder.id, value).then((_) => _loadData());
                }
              },
            ),
          ),
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        ...
                        fontSize: 13,
                        fontWeight: isPast && !reminder.isCompleted ? FontWeight.bold : null,
                      ),
                    ),
                ...
                if (reminder.repeat && reminder.frequency != null) ...[
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text('Repeats ${reminder.frequency}', style: TextStyle(color: Colors.grey[600], fontSize: 13)),
                    ],
                  ),
                ],
    ...
    switch (reminder.type) {
      ...
      case AppConstants.typeFeeding:
        return reminder.portion != null
            ? 'Portion: ${reminder.portion}' + (reminder.mealType != null ? ' | Type: ${reminder.mealType}' : '')
            : '';
      case AppConstants.typeVaccination:
  ...
  void _showAddReminderDialog() {
    if (_petsMap.isEmpty) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Please add a pet first before creating reminders')));
      return;
    }
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            Navigator.pushNamed(
              ...
              arguments: {'petId': pet.id, 'petName': pet.name, 'reminderType': type},
            ).then((_) {
              if (mounted) {
                _loadData();
              }
